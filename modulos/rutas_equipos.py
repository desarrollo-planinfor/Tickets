...
def lista_equipos():
    ...
    # Calcular responsables con más de un equipo (sobre TODOS los equipos)
    from collections import Counter
    responsable_counter = Counter(
        e.responsable.strip() for e in todos_equipos 
        if e.responsable and e.responsable.strip()
    )
    multi_responsables = {r for r, count in responsable_counter.items() if count > 1}
    ...
    return render_template('equipos/lista.html',  
                           ...
                           multi_responsables=multi_responsables,
                           current_tab=tab)
